...
(
    ...
    cd $SCRIPT_PATH/../server/export
    mkdir -p $NEWTHEME_DIRNAME && cp -r blueclaire/* $NEWTHEME_DIRNAME
    find $NEWTHEME_DIRNAME -name \*.js | while read f ; do
        echo server/export/$f
        sed -i .tmp \
            -e "s/blueclaire/$NEWTHEME_DIRNAME/g" \
            -e "s/BlueClaireTheme/$NEWTHEME_IMPORT/g" \
            -e "s/theme \"BlueClaire/theme \"$NEWTHEME_USE/g" \
            $f && rm $f.tmp
    done

    echo "* Do not forget to update www/app/themes/$NEWTHEME_DIRNAME/resources/images/*.png"
